...
// Character Description class/template. Used for form submission as a container.
...
$(function () {
    ...
    function submitCharacterEdit() {
        ...
        fetch('/api/character_edit', options).then((response) => {
            if (!response.ok) {
                ...
                window.location.replace($('#character_page').data('character_url'));
            }
        })
    }
    ...
    function submitCharacterCreation() {
        ...
        // API call
        fetch('/api/character_create', options).then((response) => {
            if (!response.ok) {
                throw new Error(`HTTP error! Status: ${response.status}`);
            } else {
                // Redirect to character page on success
    ...
    // Parses form/DOM and assembles a CharacterDescription object to be sent.
    function createCharacterData() {
        ...
        // Create character description object, assign values
        ...
        if (image_array.length > 0){
            characterDescription.image_ids = image_array;
        }
